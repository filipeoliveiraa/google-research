 ...
 public:
  explicit SddmmOp(tensorflow::OpKernelConstruction* context)
    ...
    OP_REQUIRES(
        ...
        absl::InvalidArgumentError("transpose_rhs=False not yet supported."));
  }
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    OP_REQUIRES(
        context, lhs_matrix.dim_size(1) == rhs_matrix.dim_size(1),
        absl::InvalidArgumentError("Last dim of input matrices must match."));
    OP_REQUIRES(context,
                TensorShapeUtils::IsMatrix(lhs_matrix.shape()) ||
                    lhs_matrix.dims() == 3,
                absl::InvalidArgumentError(
                    "Expected 2-dim or 3-dim lhs matrix tensor."));
    OP_REQUIRES(context,
                TensorShapeUtils::IsMatrix(rhs_matrix.shape()) ||
                    rhs_matrix.dims() == 3,
                absl::InvalidArgumentError(
                    "Expected 2-dim or 3-dim rhs matrix tensor."));
    ...
    // matrix.
    OP_REQUIRES(
        context, rhs_matrix.dims() == lhs_matrix.dims(),
        absl::InvalidArgumentError("rhs and lhs must match number of dims."));
    ...
    OP_REQUIRES(
        context, lhs_matrix.dim_size(dim_offset) == m,
        absl::InvalidArgumentError("First dim of lhs must match output rows."));
    OP_REQUIRES(
        context, rhs_matrix.dim_size(dim_offset) == n,
        absl::InvalidArgumentError("First dim of lhs must match output cols."));

    // If we're going to run multiple sddmms, the first dimension of the
    // matrices must match.
    OP_REQUIRES(
        context, replication == 1 || replication == rhs_matrix.dim_size(0),
        absl::InvalidArgumentError("First dim of lhs & rhs must match"));
