...
[Cloud Machine Learning Engine][cmle], skip to the
[Train on Cloud Machine Learning Engine](#train-on-cloud-machine-learning-engine)
section.
...
The data is expected to be formatted in TFRecord format, as generated by
[this script][imagenet-download-format-as-tfrecord].
...
1. Train the model by executing the following command (substituting the
appropriate values):
...
create it with `gcloud storage buckets create <bucket>`.) This bucket is used
to save checkpoints and the training result, so that the training steps are
cumulative when you reuse the model directory. If you do 1000 steps, for
example, and you reuse the model directory, on a subsequent run, it will skip
the first 1000 steps, because it picks up where it left off.
