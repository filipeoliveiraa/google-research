...
"""Configuration for Gemma fine-tuning on phenotyping datasets."""
...
def get_config():
  """Returns the experiment configuration for Gemma detection."""
  cfg = config.get_detection_base_config()

  cfg.experiment_name = "gemma_detection_finetune"
  cfg.model_flavor = config.ModelFlavor.GEMMA_3
  ...
  cfg.model_id = "path/to/gemma-model"

  cfg.output_dir = "/path/to/gemma_detection_output"
  ...
  # Let the Gemma 3 processor handle native image resolution. The base config
  # defaults to 840 (for OWL-v2), so we explicitly disable forced resizing.
  ...
  # Auto-falls back to fp32 on CPU.
