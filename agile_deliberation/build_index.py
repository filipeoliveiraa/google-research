...
def build_index(data_dir, max_shards = None):
  """Builds a FAISS inner-product index from .npy embedding shards.

  Loads image embedding shards from ``data_dir/npy/``, L2-normalises them,
  adds them to a flat inner-product index, and writes the index along with
  a JSON metadata file to ``data_dir``.

  Args:
    data_dir: Root directory containing a ``npy/`` subfolder with
      ``img_emb_*.npy`` shard files.
    max_shards: If given, only the first max_shards shard files are indexed.
      Defaults to ``None`` (index all shards).

  Raises:
    FileNotFoundError: If no ``img_emb_*.npy`` files are found in the
      ``npy/`` subfolder.
  """
