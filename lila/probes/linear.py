...
class LinearCat(nn.Module):
  ...
  def __init__(
    ...
    self.probe_dec = nn.Conv2d(
        idim_dec, odim, kernel_size, stride=stride, padding=padding
    )

  def parameters(self):
    return list(self.probe_dec.parameters()) + list(self.probe_enc.parameters())
  ...
  def forward(self, feats_enc, feats_dec, scale_to):
    ...
    if logits_dec.shape[-2:] != scale_to:
      logits_dec = F.interpolate(
          logits_dec, scale_to, mode="bilinear", align_corners=False
      )

    # encoder features
    logits_enc = self.probe_enc(feats_enc)
    logits_enc = F.interpolate(
        logits_enc, scale_to, mode="bilinear", align_corners=False
    )
...
class LinearZeroShot(nn.Module):
  ...
  def reg(self):
    return 1000.0 * orthogonality_loss(self.enc_proj)

  def parameters(self):
    return (
        list(self.enc_proj.parameters())
        + list(self.dec_proj.parameters())
        + [self.logit_scale]
    )
