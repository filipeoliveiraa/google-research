...
class PAMR(nn.Module):
  ...
  def __init__(self, num_iter=1, dilations=[1], resize=None, sigma=0.1):
    ...
    self.num_patches_9 = (
        9 * self.num_dilations
    )  # P_9 (total patches from 9-gather)
    ...
    self.register_buffer(
        'neighbor_inds_from_9', neighbor_inds_from_9, persistent=False
    )
  ...
  def forward(self, guide_map, target_map):
    ...
    target_map = F.interpolate(
        target_map, size=resize_to, mode='bilinear', align_corners=True
    )

    guide_map = F.interpolate(
        guide_map, size=resize_to, mode='bilinear', align_corners=True
    )
    ...
    g_8_neighbors = torch.index_select(
        g_neighbors_9, 2, self.neighbor_inds_from_9
    )
