...
class EvalSeg(EvalBase):
  ...
  def __init__(self, cfg):
    ...
    # self.ignore_cls_list = self.train_dataset.ignore_cls_list

    # loading all queries
    label_embedings = torch.load("splits/coco_embeddings.pt")
    ...
    self.probe = Probe(
        cfg.probe, cfg.probe.enc_fdim, cfg.probe.dec_fdim, label_embedings
    )
    self.probe.cuda()
    ...
    self.opt = optim.AdamW(
        self.parameters(), lr=cfg.eval.lr, weight_decay=cfg.eval.weight_decay
    )
    self.best_val = self._init_metrics()
    self._setup_v("seg")

  def _init_metrics(self):
    return {"mIoU_harmonic": 0.0}
  ...
  def validate(self, epoch):
    ...
    if eval_dict["mIoU_harmonic"] > self.best_val["mIoU_harmonic"]:
      with_aux = self.best_val["mIoU_harmonic"] == 0
      print(
          f"Best mIoU: {eval_dict['mIoU_harmonic']}. Saving (with"
          f" GT={with_aux})..."
      )
    ...
    for epoch in range(self.cfg.eval.num_epochs):
      ...
      for niter, batch in enumerate(pbar):
        ...
        loss = F.cross_entropy(
            pred_mask, gt_masks, ignore_index=self.train_dataset.ignore_index
        )

        # adding unseen categories
        loss_neg = torch.Tensor([0])
        ignore_mask = gt_masks == self.train_dataset.ignore_index

        if ignore_mask.any():
          probs = F.softmax(pred_mask, dim=1)
          p_unseen = probs[:, seen_ids, :, :].sum(dim=1)  # [B, H, W]
        ...
        pbar.set_description(
            f"Iter {niter:03d} |"
            + f" Loss={loss.item():4.3f}, Neg={loss_neg.item():4.3f}"
        )
