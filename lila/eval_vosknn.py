...
class EvalKnnVOS(EvalBase):
  ...
  def __init__(self, cfg):
    ...
    self.writer = tensorboard.SummaryWriter(
        os.path.join(cfg.runtime.root, cfg.runtime.name)
    )
  ...
  def _pad_and_unfold(self, x):
    ...
    x_unfolded = x_unfolded.unflatten(1, (d, attn_window**2))  # B,d,KK,N
    return x_unfolded

  @torch.compile(mode="reduce-overhead")
  def _propagate_labels(
      self, feats_current, feats_context_list, masks_context_list
  ):
    window_size = self.cfg.eval.vos_knn.attn_window
    H, W = feats_current.shape[-2:]

    feats_context = torch.stack(feats_context_list)  # T x d x h x w
    masks_context = torch.stack(masks_context_list)

    Q_normed = feats_current.movedim(1, -1)  # 1, H, W, d
    K_normed = feats_context.movedim(1, -1)  # T, H, W, d
    ...
    with torch.amp.autocast("cuda", enabled=True):
      ...
      V_patches = V_patches.permute(3, 0, 2, 1).flatten(1, 2)  # C, TK, N

      K_padded = F.pad(
          K_normed, (0, 0, padding, padding, padding, padding), "replicate"
      )
      ...
      for u_offset in range(window_size):
        for v_offset in range(window_size):
          # Get the slice from padded Keys corresponding to this offset
          # Shape: (T, H, W, d)
          K_slice = K_padded[
              :, u_offset : u_offset + H, v_offset : v_offset + W, :
          ]

          # Compute dot product (which is cosine sim since vectors are normed)
          # (1, H, W, d) * (T, H, W, d) -> sum(dim=3) -> (T, H, W)
          dot_prod = (Q_normed * K_slice).sum(dim=3)

          # Append the (T, H, W) tensor. We will stack them later.
          patches_list.append(dot_prod)

      # (U*U, T, H, W)
      all_sims_stacked = torch.stack(patches_list, dim=0)

      # (U*U, T, N)
      all_sims_stacked = all_sims_stacked.flatten(-2)

      # (U*U, T, N) -> (T, U*U, N) -> (N, T U*U)
      sim = all_sims_stacked.permute(2, 1, 0).flatten(1, 2)

      topk, topk_idxs = sim.topk(self.cfg.eval.vos_knn.num_nn, -1)

      topk_vals = V_patches.gather(
          1, topk_idxs.unsqueeze(-1).expand(-1, -1, V_patches.shape[-1])
      )
  ...
  def _to_onehot(mask_hw):
    ...
    # Permute the dimensions to get the desired [C, H, W] format
    # (H, W, C) -> (C, H, W)
  ...
  def _predict_with_knn(self, cfg, frames, ref_mask):
    ...
    l2norm = (
        lambda x: F.normalize(x, dim=1, p=2) / self.cfg.eval.vos_knn.temp**0.5
    )
    ...
    for frame_current in tqdm(frames[1:]):
      ...
      _context_masks = [ref_mask0_rescaled[0]] + masks_context

      mask_out = torch.zeros_like(_context_masks[0])
      ...
      for feats, ftype in zip([enc_feats, dec_feats], ["enc", "dec"]):
        ...
        _context_feats = [feats0_rescaled[ftype][0]] + feats_context[ftype]

        feats_current = l2norm(self._rescale_grid(feats))

        mask = self._propagate_labels(
            feats_current, _context_feats, _context_masks
        )
  ...
  def _rescale_grid(self, x):
    ...
    # Determine which side is smaller and calculate the scaling ratio
    if H < W:
      # If height is the smaller side, the ratio is sizeMin / H
      H_min = self.cfg.eval.vos_knn.feat_grid_size
      r = H_min / H
      W_min = int(W * r)
    else:
      # If width is the smaller side (or they are equal), the ratio is sizeMin / W
      W_min = self.cfg.eval.vos_knn.feat_grid_size
      r = W_min / W
      H_min = int(H * r)

    # print(f"Rescaled features {H}x{W} to {H_min}x{W_min}")
    x_rescaled = self._rescale(x, (H_min, W_min))
    return x_rescaled
  ...
  def eval_sequence(self, frames, masks):
    ...
    resampler = PatchResample(HW[0], HW[1], 476, patch_size=14)
    frames_sm = resampler.adjust_to_patch_size(frames)

    ref_mask_onehot = self._to_onehot(masks[:1].cuda())

    # predicting result
    pred_masks_but_first = self._predict_with_knn(
        self.cfg.eval.vos_knn, frames_sm, ref_mask_onehot
    )
  ...
  def evaluate(self):
    ...
    self.writer.add_hparams(
        dict(self.cfg.eval.vos_knn), self.davis_metrics.dict(), run_name="."
    )
    self.writer.flush()
