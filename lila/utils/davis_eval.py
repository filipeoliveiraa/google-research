...
class DavisEvaluate:
  ...
  def add_sequence(self, gt_masks, res_masks):
    ...
    if all_res_masks.shape[0] > all_gt_masks.shape[0]:
      sys.stdout.write(
          "\nIn your PNG files there is an index higher than the number of"
    ...
    for ii in range(all_gt_masks.shape[0]):
      j_metrics_res[ii, :] = db_eval_iou(
          all_gt_masks[ii, Ellipsis], all_res_masks[ii, Ellipsis], None
      )
  ...
  def dict(self):
    ...
    return {
        "JF": (np.mean(J["M"]) + np.mean(F["M"])) / 2.0,
        "Jm": np.mean(J["M"]),
        "Jr": np.mean(J["R"]),
        "Jd": np.mean(J["D"]),
        "Fm": np.mean(F["M"]),
  ...
  def print_metrics(self):
    ...
    print("J&F-Mean: {:4.3f}".format((np.mean(J["M"]) + np.mean(F["M"])) / 2.0))
...
def davis_masks(masks):
  ...
  for i in range(masks.shape[0]):
    masks[i, masks[i, Ellipsis] == 255] = 0

  # separate_objects_masks = True
  num_objects = int(np.max(masks[0, Ellipsis]))
  tmp = np.ones((num_objects, *masks.shape))
  tmp = tmp * np.arange(1, num_objects + 1)[:, None, None, None]
  masks = tmp == masks[None, Ellipsis]
  masks = masks > 0
...
def f_measure(foreground_mask, gt_mask, void_pixels=None, bound_th=0.008):
  ...
  Calculates precision/recall for boundaries between foreground_mask and gt_mask
  using morphological operators to speed it up.
...
def _seg2bmap(seg, width=None, height=None):
  ...
  Arguments: seg     : Segments labeled from 1..k. width       :     Width of
  desired bmap  <= seg.shape[1] height  : Height of desired bmap <= seg.shape[0]
